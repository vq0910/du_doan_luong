...
button_svm = Button(form, text='Kết quả dự đoán theo ID3', command=dudoan)
button_svm.grid(row=7, column=2, pady=20)


def khanang():
    y_id3 = id3.predict(xtest)
    messagebox.showinfo("Khả năng dự đoán của ID3",
                        "Độ chính xác của phương pháp: " + str(accuracy_score(ytest, y_id3) * 100) + "%")
